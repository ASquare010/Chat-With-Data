...
class Message(Base):
    ...
    base64_image = Column(Text, nullable=True)         # store small images as base64; prefer object storage for large files
    metadata = Column(JSONB, nullable=True, default={}) # e.g. {"tokens": 123, "model": "gpt-5"}
    created_at = Column(DateTime(timezone=True), server_default=func.now())
